...
RSpec.describe ForcedRate, type: :model do
  ...
  describe 'validations' do
    ...
    it { is_expected.to allow_value(Time.current + 1.second).for(:expires_at) }
    it { is_expected.not_to allow_value(Time.current).for(:expires_at) }
    it { is_expected.not_to allow_value(Time.current - 1.second).for(:expires_at) }
